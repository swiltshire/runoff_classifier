...
def main():
    ...
    # restrict search to your mask polygons
    MASK_PATH = "../data/NHDmask_Indiana/NHDfinalMaskIndiana.shp"   # <-- set this to your mask
    
    mask_raster = rasterize_mask_aligned(args.raster_path, MASK_PATH)
    windows_all = make_grid_windows(args.raster_path, args.tile_size, args.stride)
    windows_all = filter_windows_by_mask_raster(mask_raster, windows_all, min_cover_frac=0.05)

    
    # # e.g., expose as a CLI arg if you prefer
    # windows_all = filter_windows_by_mask(
    #     raster_path=args.raster_path,
    #     windows=windows_all,
    #     mask_path=MASK_PATH,
    #     min_cover_frac=0.05   # require at least 5% coverage (tune as desired)
    # )
    # if is_main_process():
    #     logger.info("[debug] masked windows=%d (from %d total)", len(windows_all), len(make_grid_windows(args.raster_path, args.tile_size, args.stride)))
