...
# CANONICAL_RES = "0.5 0.5"            # ft
# S3_BUCKET = "sagemaker-gst-stage.sharing"
# S3_PREFIX = "serge-wiltshire/runoff-classifier-data/reprojected_imagery"

# GDALWARP = "gdalwarp"                # must be on PATH
...
#     cmd = [
#         GDALWARP,
#         "-q",  # quiet mode: suppress per-file output
#         "-t_srs", CANONICAL_CRS,
#         "-tr", *CANONICAL_RES.split(),
#         "-tap",
#         "-r", "cubic",
...
#     # CRS diagnostics (once per county)
#     crs_counts = Counter(tile_epsg(t) for t in local_tiles)
#     log("  Tile CRS distribution:")
#     for crs, n in crs_counts.items():
#         log(f"    {crs}: {n}")
...
#         with tqdm(total=len(local_tiles), unit="file", desc=desc) as pbar:
#             for tile in local_tiles:
#                 tile_name = tile.name
#                 s3_key = s3_key_for_tile(county_safe, tile_name)
...
#     log(f"Preparing canonical tiles for {len(counties)} counties")
#     for c in counties:
#         ensure_canonical_tiles_for_county(c, force=force)
#     log("All requested counties processed")
